...
def main():
    ...
    for style in styles:
        ...
        style_path.mkdir(exist_ok=True, parents=True)
        print(f'Generating image with style: {style}')
        set_parameters(client, positive_prompt=positive_prompt, negative_prompt=negative_prompt, seed=seed,
                       style=[style])
        result = generate(client)
        for image in result[3]['value']:
            image_path = Path(image['name'])
            target_path = style_path / image_path.name
